...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ mes?: string }> }) {
  ...
  const receitasMes = transacoes.filter((item) => new Date(item.dataCadastro).getMonth() === mesAtual && item.tipo === "receita");
  const despesasMes = transacoes.filter((item) => new Date(item.dataCadastro).getMonth() === mesAtual && item.tipo === "despesa");
  ...
  return <main className="mx-auto flex w-full max-w-[1600px] flex-1 flex-col gap-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.9fr)]"><Card><CardHeader className="flex flex-row items-start justify-between gap-4 border-b pb-4"><div><CardTitle>Despesas mensais</CardTitle><p className="mt-1 text-sm text-muted-foreground">Comparativo entre {anoAtual} e {anoAtual - 1}</p></div><div className="flex shrink-0 gap-3 text-xs text-muted-foreground"><span className="flex items-center gap-1.5"><i className="size-2 rounded-full bg-primary" />{anoAtual}</span><span className="flex items-center gap-1.5"><i className="size-2 rounded-full bg-muted-foreground/25" />{anoAtual - 1}</span></div></CardHeader><CardContent><GraficoMensal atual={agruparPorMes(transacoes, anoAtual, "despesa")} anterior={agruparPorMes(transacoesAnteriores, anoAtual - 1, "despesa")} /></CardContent></Card>
      <Card><CardHeader><CardTitle>Onde você mais gasta</CardTitle><p className="mt-1 text-sm text-muted-foreground">Distribuição das despesas de {anoAtual}</p></CardHeader><CardContent className="space-y-5 pt-3">{despesasPorCategoria.length === 0 ? <p className="py-10 text-center text-sm text-muted-foreground">Ainda não há despesas registradas.</p> : despesasPorCategoria.map(([categoria, valor], index) => <div key={categoria}><div className="mb-2 flex justify-between gap-3 text-sm"><span className="truncate font-medium">{categoria}</span><span className="text-muted-foreground">{moeda.format(valor)}</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className={`h-full rounded-full ${["bg-primary", "bg-sky-500", "bg-violet-500", "bg-amber-500", "bg-rose-400"][index]}`} style={{ width: `${(valor / maiorCategoria) * 100}%` }} /></div></div>)}</CardContent></Card></div>
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]"><Card><CardHeader className="flex flex-row items-center justify-between border-b pb-4"><div><CardTitle>Maiores despesas</CardTitle><p className="mt-1 text-sm text-muted-foreground">As movimentações que mais impactaram seu orçamento</p></div><Link href="/transacoes?tipo=despesa" className="flex items-center text-xs font-medium text-primary hover:underline">Ver todas <ChevronRight className="ml-1 size-3" /></Link></CardHeader><CardContent className="p-0">{maioresDespesas.length === 0 ? <p className="p-6 text-sm text-muted-foreground">Nenhuma despesa registrada neste ano.</p> : <div className="divide-y">{maioresDespesas.map((item) => <div key={item.transacaoId} className="flex items-center justify-between gap-4 px-6 py-4"><div className="flex min-w-0 items-center gap-3"><div className="flex size-9 shrink-0 items-center justify-center rounded-full bg-rose-500/10 text-rose-500"><CircleDollarSign className="size-4" /></div><div className="min-w-0"><p className="truncate text-sm font-medium">{item.descricao}</p><p className="text-xs text-muted-foreground">{item.categoriaNome} · {formatarData(item.dataCadastro)}</p></div></div><span className="shrink-0 text-sm font-semibold text-rose-600">- {moeda.format(item.valor)}</span></div>)}</div>}</CardContent></Card>
      <Card className="border-0 bg-primary text-primary-foreground"><CardHeader><CardTitle className="text-primary-foreground">Fechamento do mês</CardTitle><p className="mt-1 text-sm text-primary-foreground/70">Seu desempenho em {nomesMes}</p></CardHeader><CardContent className="space-y-5"><div className="flex items-end justify-between"><div><p className="text-sm text-primary-foreground/70">Saldo disponível</p><p className="mt-1 text-3xl font-semibold">{moeda.format(saldo)}</p></div><Wallet className="size-8 text-primary-foreground/60" /></div><div className="h-2 overflow-hidden rounded-full bg-primary-foreground/20"><div className="h-full rounded-full bg-primary-foreground" style={{ width: `${Math.min(economia, 100)}%` }} /></div><div className="grid grid-cols-2 gap-3 border-t border-primary-foreground/15 pt-4 text-sm"><div><p className="text-primary-foreground/60">Entradas</p><p className="mt-1 font-medium">{moeda.format(totalReceitasMes)}</p></div><div><p className="text-primary-foreground/60">Saídas</p><p className="mt-1 font-medium">{moeda.format(totalDespesasMes)}</p></div></div><Link href="/transacoes" className="flex items-center justify-center rounded-lg bg-primary-foreground/10 px-3 py-2 text-sm font-medium transition-colors hover:bg-primary-foreground/20">Ver movimentações <ChevronRight className="ml-1 size-4" /></Link></CardContent></Card></div>
  </main>;
}
